...
You have these tools: `bash`, `read_file`, `write_file`, `edit_file`, `multi_edit`, `glob`, `grep`, `web_fetch`, `todo_write`. Use them in a loop - read what you need, make the change, check it, fix what's broken - calling as many as the task takes.
