...
4. **No browser available?** Don't count braces or grep for a function name — that's busywork that passes on broken code. Trace the init path and the first-action handler (the Start click) by hand: a symbol used before it's defined, a property that doesn't exist (`renderer.domElement`, not `camera.domElement`). State what you couldn't run on its own line — `unverified: <what> — <why>`.
