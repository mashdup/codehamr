...
4. **No runtime here?** No browser, or `node`/`npx` missing and not installable (don't hunt it) — trace the init path and the first-action handler (the Start click) by hand, a symbol used before it's defined or a property that doesn't exist (`renderer.domElement`, not `camera.domElement`), not a brace count or a symbol grep. State what you couldn't run on its own line — `unverified: <what> — <why>`.
